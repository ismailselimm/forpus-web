  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ animationDelay: FLOAT_DELAYS[index % FLOAT_DELAYS.length] }}
      >
        <div
          aria-hidden="true"
        ...
        <div className="relative w-[200px] rounded-[2.6rem] bg-ink p-[10px] shadow-[var(--shadow-card)] ring-1 ring-white/10 sm:w-[216px]">
          ...
          <div
            aria-hidden="true"
            className="absolute -left-[2px] top-[88px] h-9 w-[3px] rounded-full bg-ink/70"
          />
          <div
            aria-hidden="true"
            className="absolute -left-[2px] top-[132px] h-9 w-[3px] rounded-full bg-ink/70"
          />
          <div
            aria-hidden="true"
            className="absolute -right-[2px] top-[110px] h-12 w-[3px] rounded-full bg-ink/70"
          />
        </div>
      </div>

      {/* Caption */}
      <h3 className="mt-12 text-[1.15rem] font-bold tracking-tight text-ink">
        {app.name}
      </h3>
      <p className="mt-1.5 max-w-[15rem] text-[0.9rem] leading-relaxed text-ink-2">
        {app.tagline[lang]}
      </p>
        ...
        (app.appStore || app.googlePlay) && (
          <div className="mt-4 flex flex-wrap items-center justify-center gap-2.5">
            ...
            {app.googlePlay && (
              <StoreBadge
                href={app.googlePlay}
                src="/badges/googleplay.webp"
                alt={`${app.name} — Google Play`}
              />
            )}
          </div>
        )
